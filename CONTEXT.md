...
> Last updated: 2026-06-05 late-night session (Phase 2F deploy prep complete — signed URL upload + token-budget throttle + v4 enrichment + schema cleanup; deploy gated only on backfill completion)
...
**Total in DB (end of Fri 2026-06-05):** 1,673 active jobs (up from 1,581 Wed — new scrapes added). 860 at v4 (51%). 811 NULL remaining (TPD ceiling of 500,000 = ~415-500 enrichments/day max on free tier — backfill needs 2 more cron runs to clear). 1 v3 straggler, 1 v2 straggler.
...
⏳ **v4 enrichment backfill COMPLETE — only remaining gate.** End of Fri 2026-06-05: 860/1,673 jobs at v4 (51%). 811 NULL remaining. TPD math: ~415-500 enrichments/day max → needs 2 cron runs to clear. Saturday morning scheduled cron at 12:00 UTC + 1 manual trigger after = clears it by Sat afternoon. **Realistic deploy target: Saturday afternoon. Sunday is safety margin.**
